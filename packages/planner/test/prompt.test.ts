...
describe('the system prompt', () => {
  ...
  it('defines all seven actions with their exact JSON shapes', () => {
    for (const type of ['Tap', 'TypeText', 'Scroll', 'PressKey', 'Wait', 'Assert', 'Finish']) {
...
describe('the user prompt', () => {
  ...
  it('puts the screen last, closest to the instruction', () => {
    const user = buildUserPrompt(request());
    expect(user.indexOf('GOAL')).toBeLessThan(user.indexOf('ELEMENT LIST'));
    expect(user.trimEnd().endsWith('Reply with one JSON object.')).toBe(true);
  });
